...
func (m command) updateCUDACompatLibs(containerRoot root, hostDriverVersion string) error {
	...
	if driverMajor < compatMajor {
		...
		return m.createLdsoconfdFile(containerRoot, cudaCompatLdsoconfdFilenamePattern, resolvedCompatDir)
	}
	return nil
}
...
// contains the specified folders on each line.
func (m command) createLdsoconfdFile(in root, pattern string, folders ...string) error {
	if len(folders) == 0 {
