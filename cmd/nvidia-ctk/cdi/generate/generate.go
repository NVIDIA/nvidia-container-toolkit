...
func (m command) generateSpecs(opts *options) ([]generatedSpecs, error) {
	...
	for _, featureFlag := range opts.featureFlags {
		cdiOptions = append(cdiOptions, nvcdi.WithFeatureFlag(featureFlag))
	}
	...
	deviceSpecsByDeviceCoherrence := (deviceSpecs)(allDeviceSpecs).splitOnAnnotation("gpu.nvidia.com/coherent")

	if coherentDeviceSpecs := deviceSpecsByDeviceCoherrence["gpu.nvidia.com/coherent=true"]; len(coherentDeviceSpecs) > 0 {
		infix := ".coherent"
	...
	if noncoherentDeviceSpecs := deviceSpecsByDeviceCoherrence["gpu.nvidia.com/coherent=false"]; len(noncoherentDeviceSpecs) > 0 {
		infix := ".noncoherent"
