...
func TestToolkitInstaller(t *testing.T) {
	...
	executables := &lookup.LocatorMock{
		LocateFunc: func(s string) ([]string, error) {
			switch s {
			case "/usr/bin/nvidia-container-runtime.cdi":
				fallthrough
			case "/usr/bin/nvidia-container-runtime.legacy":
				fallthrough
			case "/usr/bin/nvidia-container-runtime":
				return []string{filepath.Join(root, s)}, nil
			case "nvidia-ctk":
				fallthrough
