...
func main() {
	...
	flags := []cli.Flag{
		...
		&cli.StringFlag{
			Name:        "host-root",
			Usage:       "the path in the container where the host filesystem is mounted",
			Value:       "/host",
			Destination: &opts.hostRoot,
...
func Install(cli *cli.Context, opts *options) error {
	...
	toolkit, err := installer.New(
		installer.WithHostRoot(opts.hostRoot),
	...
	if err := toolkit.Install(opts.toolkitRoot); err != nil {
		if !opts.ignoreErrors {
	...
	nvidiaContainerCliExecutable := filepath.Join(opts.toolkitRoot, "nvidia-container-cli")
	nvidiaCTKPath := filepath.Join(opts.toolkitRoot, "nvidia-ctk")
	nvidiaCDIHookPath := filepath.Join(opts.toolkitRoot, "nvidia-cdi-hook")
	nvidiaContainerRuntimeHookPath := filepath.Join(opts.toolkitRoot, "nvidia-container-runtime-hook")
