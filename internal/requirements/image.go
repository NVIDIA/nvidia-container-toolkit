...
// CheckImage evaluates NVIDIA_REQUIRE_* constraints using host properties.
func CheckImage(logger logger.Interface, cudaImage *image.CUDA, driver *root.Driver) error {
	if cudaImage == nil || cudaImage.HasDisableRequire() {
	...
	if driver == nil {
		driver = root.New(root.WithLogger(logger))
	}

	r := New(logger, imageRequirements)

	cudaVersion, err := cuda.Version()
	if err != nil {
		logger.Warningf("Failed to get CUDA version: %v", err)
	} else {
		r.AddVersionProperty(CUDA, cudaVersion)
	}

	computeCapability, err := cuda.ComputeCapability(0)
	if err != nil {
		logger.Warningf("Failed to get CUDA Compute Capability: %v", err)
	} else {
		r.AddVersionProperty(ARCH, computeCapability)
	}
	...
	if err != nil {
		...
		if normErr != nil {
			...
			r.AddVersionProperty(DRIVER, normalized)
		}
	}
	...
	if err != nil {
		...
		r.AddStringProperty(BRAND, brand)
	}
