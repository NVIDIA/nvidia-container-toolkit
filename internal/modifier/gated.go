...
const (
	nvidiaDotSoSymlinksEnvvar = "NVIDIA_DOT_SO_SYMLINKS"
...
func NewFeatureGatedModifier(logger logger.Interface, cfg *config.Config, image image.CUDA, driver *root.Driver) (oci.SpecModifier, error) {
	...
	if image.Getenv(nvidiaDotSoSymlinksEnvvar) == "enabled" {
		version, err := driver.Version()
		if err != nil {
			return nil, fmt.Errorf("failed to get driver version required for NVIDIA_DOT_SO_SYMLINKS=enabled: %w", err)
		}
		d := discover.CreateNvidiaCTKHook(
