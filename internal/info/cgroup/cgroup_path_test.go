...
func TestParseCgroupProcFile(t *testing.T) {
	testCases := []struct {
		...
		{
			description: "hybrid hierarchy selects the unified entry",
			contents:    "12:devices:/docker/abc123\n0::/docker/abc123\n",
		...
		{
			description: "path containing a colon",
			contents:    "0::/foo:extra",
			expected:    filepath.Join(rootDirectory, "/foo:extra"),
		},
	}
